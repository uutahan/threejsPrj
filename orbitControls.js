...
THREE.OrbitControls = function ( object, domElement ) {
	...
	var updateMouse3D = function () {
		...
		return function updateMouse3D( event ) {
			...
			mouse.x = ( event.offsetX / scope.domElement.clientWidth ) * 2 - 1;
			mouse.y = - ( event.offsetY / scope.domElement.clientHeight ) * 2 + 1;

			raycaster.setFromCamera( mouse, scope.object );

			//when zooming if scene is set, we can zoom to point on the cube.
			if ( scope.scene !== null ) {
				...
				if ( intersects.length > 0 ) {
					...
					//if we intersect with any object, update mouse3d with object's hit point
					//this is same for both orthographic and perspective cameras so we can return immediately.
			...
			if ( scope.object.isPerspectiveCamera ) {
				...
				var distance = ( scope.target.y - scope.object.position.y ) / v.y; 
				...
				mouse3D.copy( origin ).add( dir.multiplyScalar( distance ) );
